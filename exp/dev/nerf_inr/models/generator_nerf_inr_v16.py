...
class GeneratorNerfINR(GeneratorNerfINR_base):
  def __init__(self,
    ...
    # self.siren = NeRFNetwork_sigma(**nerf_cfg)
    ...
    # inr_net
    # self.inr_net = INRNetwork_Skip(**{**inr_cfg,
    #                                   "input_dim": self.siren.rgb_dim})
    self.inr_net = INRNetwork_Skip_LinearSinMod(**{**inr_cfg,
                                                   "input_dim": self.siren.rgb_dim})
    self.mapping_network_inr = multi_head_mapping.MultiHeadMappingNetwork(
      use_equal_fc=False, lr_mlp=1., scale=1., norm_weight=False, add_norm=True,
      **{**mapping_inr_cfg,
         'head_dim_dict': self.inr_net.style_dim_dict,
         })
    # self.mapping_network_inr = multi_head_mapping.MultiHeadMappingNetwork_EqualLR(
    #   **{**mapping_inr_cfg,
    #      'head_dim_dict': self.inr_net.style_dim_dict,
    #      })
    # self.mapping_network_inr.apply(multi_head_mapping.kaiming_leaky_init_elr)
  ...
  def z_sampler(self,
    ...
    # print ('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    # if dist == 'gaussian':
    z = 0.01 * torch.randn(shape, device=device) 
